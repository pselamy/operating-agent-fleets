...
- [ ] Fetch every public branch, tag, pull-request head, and pull-request merge ref into a non-shallow, non-promisor clone, then run `python3 tools/verify_history_exception_boundary.py --remote origin --current <full-candidate-commit-oid> [--attestation <physically-separate-H1-attestation>]`. The comparator first proves every fetched local ref equals the evolving remote ref, scans the complete ref set with Git replacement objects disabled, and compares exact finding identities/categories/counts/digest with the closed boundary. It also proves the current commit contains the exact boundary; a ready candidate descends from a real pending base and changes exactly the declared activation paths. Pending or unattested ready state returns `MATCH BUT NOT EXTERNALLY AUTHORIZED`; only an exact external attestation bound to the exact ready commit can return `PASS WITH RECORDED HISTORICAL EXCEPTIONS`. Persist only aggregate/digest output. The standalone scanner's object/line diagnostics are controlled local investigation data and must not enter public logs or artifacts.
...
A `REVISE`, mismatch, operational failure, or not-externally-authorized result blocks merge or publication. Absence of a recorded result is not approval. The ready policy candidate requires Patrick's second H1 decision against its immutable commit before merge.

`PASS WITH RECORDED HISTORICAL EXCEPTIONS` is available only when a ready candidate's exact full-scan comparison and physically separate exact-commit H1 attestation both pass. It does not authorize H2/H3/H4, sensitive-case processing, deployment, or publication.
